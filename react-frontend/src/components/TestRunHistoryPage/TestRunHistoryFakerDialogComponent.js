...
import testRunHistoryFakerFactory from "./testRunHistoryFakerFactory";
...
const TestRunHistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = testRunHistoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("testRunHistory").create(elem));
...
export default connect(mapState, mapDispatch)(TestRunHistoryFakerDialogComponent);
