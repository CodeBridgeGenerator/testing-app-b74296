...
import seederData from "./testRunHistory.seeder.json";
...
const TestRunHistorySeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("testRunHistory").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(TestRunHistorySeederDialogComponent);
